...
/** Get graph path corresponding to this search state.
 * @return graph path corresponding to this search state
 */
fawkes::NavGraphPath &
...
NavGraphSearchState::children()
{
  ...
  for (unsigned int i = 0; i < descendants.size(); ++i) {
	...
	expand = false;
        } else if (constraint_repo_->blocks(path_.nodes().back(), d)) {
	expand = false;
      }
    }
    ...
    if (expand) {
      ...
      if (constraint_repo_) {
	float cost_factor = 0.;
	if (constraint_repo_->increases_cost(path_.nodes().back(), d, cost_factor)) {
	  d_cost *= cost_factor;
	}
      }
      // TODO: better conversion from const vector to vector
      std::vector<fawkes::NavGraphNode> non_const_nodes = path_.nodes();
      NavGraphPath * new_path = new NavGraphPath(& path_.graph(), non_const_nodes, path_.cost());
      new_path->add_node(d);
