...
/** @class AStarSpeed <plugins/colli/search/astar_speed.h>
 ...
 */
AStarSpeed::AStarSpeed( LaserOccupancyGrid * occGrid, Logger* logger, Configuration* config )
 : logger_( logger )
{
  logger_->log_debug("AStarSpeed", "(Constructor): Initializing AStarSpeed");

  max_states_       = config->get_int( "/plugins/colli/search/a_star/max_states" );
  cfg_turn_factor_  = config->get_float( "/plugins/colli/search/a_star/a_star_speed/turn_factor" );
  ...
  logger_->log_debug("AStarSpeed", "(Constructor): Initializing AStarSpeed done");
}
 ...
 */
AStarSpeed::~AStarSpeed()
{
  logger_->log_debug("AStarSpeed", "(Destructor): Destroying AStarSpeed");
  for( int i = 0; i < max_states_; i++ )
    delete astar_states_[i];
  logger_->log_debug("AStarSpeed", "(Destructor): Destroying AStarSpeed done");
}
...
AStarSpeed::search( )
{
  ...
  while ( open_list_.size() > 0 ) {
    ...
    if ( is_goal( best ) )
      ...
      logger_->log_warn("AStarSpeed", "**** Warning: Out of states! Increasing A* MaxStates!");
      ...
      logger_->log_warn("AStarSpeed", "**** Increasing done!");
      return 0;
    }
 ...
 */
long
AStarSpeed::calculate_key( int x, int y, int p_x, int p_y )
{
  long l_x = x;
  long l_y = y;
  long l_p_x = p_x;
  long l_p_y = p_y;
  return (l_p_x << 43) | (l_p_y << 29) | (l_x << 15) | l_y;  // This line is a crime! But fast ;-)
}
 ...
 */
void
AStarSpeed::generate_children( AStarState * father )
{
  AStarState * child;
  ...
  if ( father->father_ ) {
    if ( father->father_->x_ > father->x_ ) { x_neg = true; }
    if ( father->father_->x_ < father->x_ ) { x_pos = true; }
    if ( father->father_->y_ > father->y_ ) { y_neg = true; }
    if ( father->father_->y_ < father->y_ ) { y_pos = true; }
  }
  int key;
  ...
  if ( father->y_ > 0 ) {
    ...
    if ( prob < cell_costs_.occ ) {	// ! prob >= cell_costs_.occ
      ...
      key = calculate_key( child->x_, child->y_, father->x_, father->y_ );
      if ( closed_list_.find( key ) == closed_list_.end() ) {
	if (x_pos or x_neg) { prob = cfg_turn_factor_         * prob; }
	if ( y_pos )        { prob = ( cfg_turn_factor_ * 2 ) * prob; }
  ...
  if ( father->y_ < (signed int)height_ ) {
    ...
    if ( prob < cell_costs_.occ ) {	// ! prob >= cell_costs_.occ
      ...
      if ( closed_list_.find( key ) == closed_list_.end() ) {
	if (x_pos or x_neg) { prob = cfg_turn_factor_ * prob; }
	if ( y_neg )        { prob = ( cfg_turn_factor_ * 2 ) * prob; }
  ...
  if ( father->x_ > 0 ) {
    ...
    if ( prob < cell_costs_.occ ) {	// ! prob >= cell_costs_.occ
      ...
      key = calculate_key( child->x_, child->y_, father->x_, father->y_ );
      if ( closed_list_.find( key ) == closed_list_.end() ) {
	if (y_pos or y_neg) { prob = cfg_turn_factor_ * prob; }
	if ( x_pos )        { prob = ( cfg_turn_factor_ * 2 ) * prob; }
  ...
  if ( father->x_ < (signed int)width_ ) {
    ...
    if ( prob < cell_costs_.occ ) {	// ! prob >= cell_costs_.occ
      ...
      if ( closed_list_.find( key ) == closed_list_.end() ) {
	if (y_pos or y_neg) { prob = cfg_turn_factor_ * prob; }
	if ( x_neg )        { prob = ( cfg_turn_factor_ * 2 ) * prob; }
 ...
 */
int
AStarSpeed::heuristic( AStarState * state )
{
 ...
 */
bool
AStarSpeed::is_goal( AStarState * state )
{
 ...
 */
point_t
AStarSpeed::remove_target_from_obstacle( int target_x, int target_y, int step_x, int step_y  )
{
  // initializing lists...
  while ( open_list_.size() > 0 )
    open_list_.pop();
  ...
  while ( !(open_list_.empty()) && (astar_state_count_ < max_states_ - 6) ) {
    ...
    open_list_.pop();
    if ( father->father_ ) {
      key = calculate_key( father->x_, father->y_, father->father_->x_, father->y_);
    } else {
      key = calculate_key( father->x_, father->y_, 0, 0);
    }
