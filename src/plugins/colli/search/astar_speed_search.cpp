...
#include "astar_speed_search.h"
#include "astar_speed.h"
#include "og_laser.h"
 ...
 */
SearchAStarSpeed::SearchAStarSpeed( LaserOccupancyGrid * occ_grid, Logger* logger, Configuration* config)
   ...
   logger_( logger )
{
  ...
  astar_ = new AStarSpeed( occ_grid, logger, config );
  logger_->log_debug("search", "(Constructor): Exiting");
}

/** Destructor */
SearchAStarSpeed::~SearchAStarSpeed()
{
  delete astar_;
}
 ...
 * @param target_y Target y position in grid
   */
void
SearchAStarSpeed::update( int robo_x, int robo_y, int target_x, int target_y, float robo_vx, float robo_vy, float robo_omega )
{
...
point_t
SearchAStarSpeed::calculate_local_target()
{
...
point_t
SearchAStarSpeed::adjust_waypoint( const point_t &local_target )
{
  return local_target;
}
...
point_t
SearchAStarSpeed::calculate_local_trajec_point( )
{
...
bool
SearchAStarSpeed::is_obstacle_between( const point_t &a, const point_t &b, const int maxcount )
{
  if (a.x == b.x && a.y == b.y)
    return false;
