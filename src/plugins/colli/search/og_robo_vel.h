...
/***************************************************************************
 *  og_robo_vel.h - Occupancy grid for colli's A* search
 *
...
#ifndef __PLUGINS_COLLI_SEARCH_OG_ROBO_VEL_H_
#define __PLUGINS_COLLI_SEARCH_OG_ROBO_VEL_H_
...
class OccupancyGridRobotVelocity : public OccupancyGrid
{
 public:
  OccupancyGridRobotVelocity( BlackBoardInterfaceListMaintainer *ifs_velocity, Logger* logger, Configuration* config, tf::Transformer* listener,
                      int width = 150, int height = 150, int cell_width = 5, int cell_height = 5);
  ~OccupancyGridRobotVelocity();
 ...
 private:
  struct RobotVelocity {
    ...
    cart_coord_2d_t velocity;
    double          radius;
  };

  void update_robot_velocitys();
  ...
  std::list<RobotVelocity> velocitys_;
